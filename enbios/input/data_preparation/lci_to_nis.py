...
from enbios2.models.prepare import LCISpoldDefinition
...
def _get_spold_files_from_nis_file2(nis_file_path: str):
    cols = ["EcoinventFilename", "EcoinventCarrierName", "Processor"]
    try:
        df = pd.read_excel(nis_file_path, engine='openpyxl', sheet_name="bare-processors", header=0)
        # drop all but the following columns
        df = df[cols]

    except ValueError as err:
        raise Exception(f"{err}, please check the NIS file '{nis_file_path}'")
    except Exception as err:
        raise Exception(f"Error reading NIS file '{nis_file_path}': {err}")
    spolds = []

    # check if the 3 column exists
    if not all([col in df.columns for col in cols]):
        # list the columns that are missing
        missing_cols = [col for col in cols if col not in df.columns]
        raise Exception(f"Columns {missing_cols} not found in NIS file '{nis_file_path}'")

    for _, row in df.iterrows():
        spolds.append(LCISpoldDefinition(**row))
...
def _lci_to_dataframe(lci_in: dict, file_name: str):
    try:
...
def _find_main_output(aggregate_exchanges, default_output_interface: str):
    """ Find main output """
    _main_output = None
...
def _generate_commands(iface_types, procs, ifaces, default_output_interface:str):
...
def spold2nis(default_output_interface: str,
    ...
    spolds = []
    if correspondence:
        _ = _get_spold_files_from_correspondence_file(correspondence)
        spolds.extend(_)
    if nis_base_path:
    ...
    for spold in spolds:
        ...
        df, dfi = _lci_to_dataframe(lci, found_name)
        if df is None:
            continue
        ...
        # Find main output
        main_output, main_output_is_output = _find_main_output(df, default_output_interface)
    ...
    cmds = _generate_commands(interface_types, processors, interfaces, default_output_interface)
...
def spold2nis2(default_output_interface: str,
              lci_base: str, correspondence,
              nis_base_path: Optional[str],
              nis_base_url: Optional[str],
              output_file: str,
    ...
    # Read the list of Spold files to process
    spolds: list[LCISpoldDefinition] = []
    if correspondence:
        # _ = _get_spold_files_from_correspondence_file(correspondence)
        # spolds.extend(_)
    ...
    if nis_base_url:
        # _ = _get_spold_files_from_nis_file_url(nis_base_url)
        # spolds.extend(_)
        raise NotImplementedError("nis_base_url file not implemented currently")
    ...
    # Process each .spold file
    # glob in lci_base all spold files
    files_in_lci_base = list(Path(lci_base).glob("*.spold"))
    filenames_in_lci_base = [f.name for f in files_in_lci_base]
    # check if all spold files are in the lci_base
    for spold_filename in spolds:
        if spold_filename.EcoinventFilename not in filenames_in_lci_base:
            raise FileNotFoundError(f"File {spold_filename.EcoinventFilename} (for processor: {spold_filename.Processor}"
                                    f" not found in {lci_base}")

    declared_spold_files = set([spold.EcoinventFilename for spold in spolds])

    for spold_filename in declared_spold_files:
        file_name = os.path.join(f"{lci_base}", f"{spold_filename}")
        ...
        processors[nis_name] = dict(name=p_name, lca_code=lci["activity"],
                                    lca_type=lci["activity type"], lca_file=lci["filename"],
                                    carrier_name=spold_filename.get("ecoinvent_carrier_name", ""),
                                    description=lci["comment"])
